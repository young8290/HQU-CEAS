...
import { PERSONAL_FORM_DETAIL_CATEGORIES, SCORE_CATEGORIES, type ScoreCategory } from '../config/scoreRules.js';
...
const personalDetailSheets: Array<{ category: ScoreCategory; sheetName: string; description: string }> = [
  { category: 'moral', sheetName: '德育测评', description: '填写德育测评加分事项，例如思想品德、纪律表现、集体活动等。' },
...
function buildPersonalDetailSheet(worksheet: ExcelJS.Worksheet, item: { category: ScoreCategory; sheetName: string; description: string }) {
  ...
  worksheet.getCell('A1').value = `${rule.label}加分明细`;
  ...
  worksheet.getCell('A4').value = '加分事项';
  worksheet.getCell('B4').value = '加分分数';
  ...
  for (let row = detailStartRow; row <= detailEndRow; row += 1) {
    ...
    worksheet.getCell(row, 2).dataValidation = {
      ...
      showErrorMessage: true,
      errorStyle: 'stop',
      errorTitle: '加分分数不符合规范',
      error: `${rule.label}加分分数必须为 0 到 ${maxValue} 之间的数字，最小单位为 ${rule.step}，合计不能超过满分。`,
      showInputMessage: true,
      promptTitle: rule.label,
      prompt: `请填写单项加分分数，合计不能超过 ${maxValue} 分。`,
    };
  }

  worksheet.getCell(`A${detailTotalRow}`).value = '模块合计';
  ...
  worksheet.getCell(`A${detailCheckRow}`).value = '检查结果';
  worksheet.getCell(`B${detailCheckRow}`).value = { formula: `IF(B${detailTotalRow}<=${maxValue},"通过","合计超过满分")` };
  worksheet.getCell(`B${detailCheckRow}`).alignment = { vertical: 'middle', horizontal: 'center' };

  [detailTotalRow, detailCheckRow].forEach((rowNumber) => {
  ...
  for (let row = 1; row <= detailCheckRow; row += 1) {
    for (let column = 1; column <= 2; column += 1) {
