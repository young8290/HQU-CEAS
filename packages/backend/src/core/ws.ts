...
import * as scoreService from '../modules/evaluation/services/scoreService.js';
import * as academicYearService from '../modules/platform/services/academicYearService.js';
import * as scoreReviewInviteService from '../modules/evaluation/services/scoreReviewInviteService.js';
      ...
      try {
        ...
        switch (msg.type) {
          ...
          case 'score:update': {
            ...
            // 管理员修改分数默认关闭（防误改），可在系统设置中开启；monitor 不受影响。
            if (ws.user!.role === 'admin') {
              const { getEntryStatus } = await import('../modules/platform/services/systemSettingService.js');
              const { allowAdminScoreEditing } = await getEntryStatus();
              if (allowAdminScoreEditing === false) {
                ws.send(JSON.stringify({
                  type: 'score:error',
                  studentId,
                  category,
                  error: '管理员修改分数功能已关闭，请在系统设置中开启',
                }));
                return;
              }
            }
            ...
            const { SCORE_CATEGORIES } = await import('../modules/evaluation/rules/scoreRules.js');
