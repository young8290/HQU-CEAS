...
import { evaluateAwardCandidate, type AwardType } from '../rules/awardRules.js';
...
async function loadClassScoreContext(classId: number, academicYearId: number) {
  ...
  const academicRanks = rankByScoreDesc(items.map((item) => ({
    studentId: item.student.id,
    value: scoreValue(item.scores, 'academic'),
  })));
  const totalRanks = rankByScoreDesc(items.map((item) => ({
    studentId: item.student.id,
