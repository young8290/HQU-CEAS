...
import { evaluateHonorCandidate, type HonorType } from '../rules/honorRules.js';
...
export async function getHonorCandidates(data: {
  ...
  return cacheService.memo('honorCandidates', key, 30 * 1000, async () => {
    ...
    const academicRanks = rankByScoreDesc(items.map((item) => ({ studentId: item.student.id, value: getScore(item.scores, 'academic') })));
    const totalRanks = rankByScoreDesc(items.map((item) => ({ studentId: item.student.id, value: getScore(item.scores, 'total') })));
    ...
    return items.map(({ student, scores }) => {
      ...
      const evaluation = evaluateHonorCandidate({
        ...
        sportsBaseScore: getScore(scores, 'sports_base'),
