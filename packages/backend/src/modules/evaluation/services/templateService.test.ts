...
test('createTemplateWorkbook creates multi-sheet personal form detail workbook', async () => {
  ...
  // 德育测评为减分口径：明细为扣分事项，最终得分 = 100 − 扣分合计。
  assert.equal(moralSheet.getCell('A1').value, '德育测评减分明细');
  assert.equal(moralSheet.getCell('A4').value, '减分事项');
  assert.equal(moralSheet.getCell('B4').value, '减分分数');
  ...
  assert.equal(moralSheet.getCell('A20').value, '扣分合计');
  assert.deepEqual(moralSheet.getCell('B20').value, { formula: 'SUM(B5:B19)' });
  assert.equal(moralSheet.getCell('A21').value, '德育测评最终得分');
  assert.deepEqual(moralSheet.getCell('B21').value, { formula: '100-SUM(B5:B19)' });
  assert.equal(moralSheet.getCell('A22').value, '检查结果');
  assert.deepEqual(moralSheet.getCell('B22').value, { formula: 'IF(B20<=100,"通过","扣分合计超过100")' });
  // 其余 6 张明细表保持加分口径不变。
  assert.equal(innovationSheet.getCell('A1').value, '创新与实践能力加分明细');
  assert.equal(innovationSheet.getCell('A4').value, '加分事项');
  assert.equal(innovationSheet.getCell('B4').value, '加分分数');
  assert.equal(innovationSheet.getCell('A20').value, '模块合计');
  assert.deepEqual(innovationSheet.getCell('B20').value, { formula: 'SUM(B5:B19)' });
  assert.deepEqual(innovationSheet.getCell('B21').value, { formula: 'IF(B20<=13,"通过","合计超过满分")' });
