...
export function useScores(classId: number | null) {
      ...
      debounceTimers.current.set(
        ...
        setTimeout(async () => {
          ...
          try {
            ...
            if (!sent) {
              const data = await api.put('/evaluation/scores', { studentId, category, value, remark });
              setStudents((prev) =>
                prev.map((s) => (
                  s.id === studentId
  ...
  const loadScoreDetails = useCallback(async (studentId: number, category: string) => {
    if (!classId) return [];
    const data = await api.get<{ details: ScoreDetailData[] }>(`/evaluation/scores/student/${studentId}/${category}/details`);
    return data.details || [];
  }, [classId]);
  ...
  const saveScoreDetails = useCallback(async (studentId: number, category: string, items: ScoreDetailData[]) => {
    ...
    setSaveError(null);
    try {
      const data = await api.put(`/evaluation/scores/student/${studentId}/${category}/details`, { items });
      setSaveStatus('saved');
      setLastSaved(new Date().toLocaleTimeString());
      setStudents((prev) =>
        prev.map((s) => {
