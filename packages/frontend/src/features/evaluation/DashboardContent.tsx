...
export default function DashboardContent() {
  ...
  useEffect(() => {
    ...
    const loadStats = async () => {
      try {
        const years = await api.get('/platform/academic-years');
        ...
        const grades = await api.get('/platform/grades');
        const gradeClasses = await Promise.all(
          grades.map((grade: any) => api.get(`/platform/grades/${grade.id}/classes`)),
        );
      ...
      <section className="rounded-lg border border-[#ded6c8] bg-[#fffaf2] p-5 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
          <ActionCard
            ...
            href={isAdmin ? '/evaluation/scores' : '/evaluation/class/scores'}
          />
          {isAdmin && (
            <>
              <ActionCard
                ...
                href="/evaluation/import"
              />
              <ActionCard
                title="导出附件"
                description="附件 2、附件 4 和失败记录。"
                href="/evaluation/export"
              />
              <ActionCard
                title="学生管理"
                description="学生、班级、年级和班级类别。"
                href="/evaluation/students"
              />
              <ActionCard
                title="系统设置"
