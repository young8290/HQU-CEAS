...
export default function SettingsPage() {
  ...
  const [entryStatus, setEntryStatus] = useState({
    ...
    // 管理员修改分数默认关闭（防误改），可在此自主开启；班长不受影响。
  ...
  useEffect(() => {
    loadYears();
    if (isAdmin) {
      api.get('/platform/system/settings')
        .then((data) => {
          if (data.entryStatus) setEntryStatus((prev) => ({ ...prev, ...data.entryStatus }));
        })
        .catch(() => undefined);
    }
  }, []);
  ...
  const loadYears = async () => {
    try {
      const data = await api.get('/platform/academic-years');
      setYears(data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleCreateYear = async () => {
    ...
    setError('');
    try {
      await api.post('/platform/academic-years', { name: newYearName });
      setNewYearName('');
  ...
  const handleActivateYear = async (yearId: number) => {
    try {
      await api.put(`/platform/academic-years/${yearId}/activate`, {});
      setSuccessMsg('学年已激活');
      await loadYears();
      setTimeout(() => setSuccessMsg(''), 3000);
    } catch (err: any) {
  ...
  const handleChangePassword = async () => {
    ...
    setError('');
    try {
      await api.put('/platform/auth/password', { oldPassword, newPassword });
  ...
  const handleSaveEntryStatus = async () => {
    try {
      await api.put('/platform/system/settings', { entryStatus });
        ...
        <div className="rounded-lg border border-[#ded6c8] bg-[#fffaf2] p-6 dark:border-neutral-800 dark:bg-neutral-900">
          ...
          <div className="space-y-4">
            ...
            <label className="flex items-start gap-3 text-sm text-neutral-700 dark:text-neutral-300">
              <input
                type="checkbox"
                checked={entryStatus.allowAdminScoreEditing}
                onChange={(event) => setEntryStatus({ ...entryStatus, allowAdminScoreEditing: event.target.checked })}
                className="mt-0.5 h-4 w-4 accent-primary-600"
              />
              <span>
                允许管理员修改分数
                <span className="mt-0.5 block text-xs text-neutral-400 dark:text-neutral-500">
                  默认关闭以防止管理员误改分数；关闭时仅限制管理员账号，班长填写与批量导入不受影响。
                </span>
              </span>
            </label>
