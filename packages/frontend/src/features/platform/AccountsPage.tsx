...
export default function AccountsPage() {
  ...
  const handleGenerate = async () => {
    ...
    setError('');
    try {
      const url = selectedGrade > 0 ? `/platform/users/generate-monitors/${selectedGrade}` : '/platform/users/generate-monitors';
  ...
  const handleResetPassword = async (userId: number, username: string) => {
    ...
    try {
      const result = await api.post(`/platform/users/${userId}/reset-password`, {});
  ...
  const handleExportAccounts = async () => {
    try {
      await api.download('/evaluation/export/accounts', '账号列表.xlsx');
      setSuccessMsg('已导出');
      setTimeout(() => setSuccessMsg(''), 3000);
    } catch (err: any) {
  ...
  const handleExportGenerateResult = async () => {
    ...
    try {
      ...
      const resp = await fetch(`/api/platform/users/export-accounts`, {
        method: 'POST',
        headers: {
  ...
  const handleSaveEmail = async (userId: number) => {
    try {
      await api.put(`/platform/users/${userId}/email`, { email: emailDrafts[userId] || null });
  ...
  const handleSendMonitorAccounts = async () => {
    ...
    try {
      const result = await api.post('/platform/mail/send-monitor-accounts', {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Generate Modal */}
      {showGenerate && (
        <Modal title="批量生成班长账号" onClose={() => setShowGenerate(false)}>
      ...
      {/* Create Admin Modal */}
      {showCreateAdmin && (
        <Modal title="新增管理员账号" onClose={() => setShowCreateAdmin(false)}>
