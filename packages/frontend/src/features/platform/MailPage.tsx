...
export default function MailPage() {
  ...
  async function load() {
    const [settingData, logData] = await Promise.all([
      api.get('/platform/mail/settings'),
      api.get('/platform/mail/logs'),
    ]);
  ...
  async function saveSettings() {
    try {
      await api.put('/platform/mail/settings', form);
      setMessage('邮件配置已保存');
      load();
    } catch (error: any) {
  ...
  async function sendTest() {
    try {
      await api.post('/platform/mail/settings/test', { recipientEmail: testEmail });
      setMessage('测试邮件已发送');
      load();
    } catch (error: any) {
      setMessage(error.message);
  ...
  async function retry(logId: number) {
    try {
      await api.post(`/platform/mail/logs/${logId}/retry`);
      setMessage('邮件已重新发送');
      load();
    } catch (error: any) {
      setMessage(error.message);
