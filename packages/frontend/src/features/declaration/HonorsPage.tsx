...
export default function HonorsPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api.get<Candidate[]>(`/declaration/honors/candidates/${classId}?honorType=${honorType}`, { forceRefresh: true })
      .then((data) => {
        setCandidates(data);
    ...
    api.get(`/declaration/honor-declarations/class/${classId}`, { forceRefresh: true })
      .then(setDeclaration)
  ...
  async function submitDeclaration() {
    ...
    try {
      await api.post('/declaration/honor-declarations', {
        classId,
        honorType,
      ...
      setDeclaration(await api.get(`/declaration/honor-declarations/class/${classId}`, { forceRefresh: true }));
    } catch (error: any) {
...
function PdfState({ declaration }: { declaration: any }) {
  ...
  return (
    <div className="rounded-lg border border-neutral-200 p-3 text-sm dark:border-neutral-800">
      ...
      <button
        type="button"
        onClick={() => api.download(`/platform/pdf-materials/${pdf.id}/download`, `荣誉称号确认协议-${pdf.id}.pdf`)}
