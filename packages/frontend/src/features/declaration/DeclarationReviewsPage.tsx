...
export default function DeclarationReviewsPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const data = await api.get('/declaration/declaration-reviews');
      setItems(data.data || []);
    } catch (error: any) {
  ...
  async function openDetail(id: number) {
    const detail = await api.get(`/declaration/declaration-reviews/${id}`);
    setSelected(detail);
  ...
  async function review(action: 'return' | 'approve') {
    if (!selected) return;
    try {
      await api.post(`/declaration/declaration-reviews/${selected.id}/${action}`, {
        opinion,
            ...
            <div className="overflow-x-auto">
              ...
              {items.length === 0 && (
                <div className="px-4 py-10 text-center">
                  <p className="text-sm font-medium text-neutral-600 dark:text-neutral-300">暂无待审核申报</p>
                  <p className="mt-2 text-sm leading-6 text-neutral-500 dark:text-neutral-400">
                    下一步：先到「申报数据导入」导入外部奖项、院奖名额和补充信息；班长在申报页提交后，记录会出现在这里等待审核。
                  </p>
                  <div className="mt-4 flex justify-center">
                    <AppLink to="/declaration/import" className="rounded-md bg-[#9a5b3d] px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-[#7c4a34]">
                      前往申报数据导入
                    </AppLink>
                  </div>
                </div>
  ...
  return (
    <div className="rounded-lg border border-[#ded6c8] bg-[#fffaf2] px-3 py-2 text-sm dark:border-neutral-800 dark:bg-neutral-950">
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        ...
        <button
          type="button"
          onClick={() => api.download(`/platform/pdf-materials/${pdf.id}/download`, `班长确认协议-${pdf.id}.pdf`)}
