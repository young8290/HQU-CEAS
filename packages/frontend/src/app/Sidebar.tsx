...
const navGroups: NavGroup[] = [
  {
    ...
    items: [
      ...
      { name: '分数管理', href: '/evaluation/scores', icon: <IconScores />, adminOnly: true },
      { name: '本班综测', href: '/evaluation/class/scores', icon: <IconScores />, monitorOnly: true },
      { name: '审核小组确认', href: '/evaluation/class/review', icon: <IconDeclaration />, monitorOnly: true },
      { name: '学生管理', href: '/evaluation/students', icon: <IconStudents />, adminOnly: true },
      { name: '数据导入', href: '/evaluation/import', icon: <IconImport /> },
      { name: '附件导出', href: '/evaluation/export', icon: <IconExport /> },
    ],
  },
  {
    label: '申报系统',
    items: [
      ...
      { name: '申报数据导入', href: '/declaration/import', icon: <IconImport /> },
      { name: '申报材料导出', href: '/declaration/export', icon: <IconExport />, adminOnly: true },
      { name: '奖学金申报', href: '/declaration/class/awards', icon: <IconDeclaration />, monitorOnly: true },
      { name: '荣誉称号', href: '/declaration/class/honors', icon: <IconDeclaration />, monitorOnly: true },
      { name: '提交记录', href: '/declaration/class/submissions', icon: <IconDeclaration />, monitorOnly: true },
      { name: '标签视图', href: '/declaration/tags', icon: <IconDeclaration />, adminOnly: true },
    ],
  },
  {
    label: '平台',
    items: [
      { name: '系统入口', href: '/entry', icon: <IconDashboard /> },
      { name: '新手指引', href: '/guide', icon: <IconGuide /> },
      { name: '账号管理', href: '/accounts', icon: <IconAccounts />, adminOnly: true },
      { name: '邮箱配置', href: '/accounts-mail', icon: <IconMail />, adminOnly: true },
      { name: '邮件模板', href: '/mail-templates', icon: <IconMail />, adminOnly: true },
      { name: '操作日志', href: '/audit-logs', icon: <IconDeclaration />, adminOnly: true },
        ...
        <nav className="flex-1 overflow-y-auto py-4">
          {visibleGroups.map((group, groupIndex) => (
            <div key={group.label} className={groupIndex > 0 ? 'mt-4' : ''}>
              {!collapsed ? (
                <p className="px-5 pb-1.5 text-[11px] font-medium uppercase tracking-[0.12em] text-neutral-400 dark:text-neutral-500">
                  {group.label}
                </p>
              ) : (
                groupIndex > 0 && <div className="mx-3 mb-3 border-t border-[#e8dcc9] dark:border-neutral-800" />
              )}
