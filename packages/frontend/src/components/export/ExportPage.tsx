...
export default function ExportPage({ scope = 'evaluation' }: { scope?: ExportScope }) {
  ...
  const handleExport = async (type: string) => {
    if (!selectedClass && !['all', 'accounts', 'failed', 'declarations', 'awardAllocation', 'honorDeclarations', 'declarationAttachment2', 'mailLogs'].includes(type)) return;
    setExporting(true);
    setError('');
    setSuccessMsg('');
    ...
    try {
      ...
      switch (type) {
        ...
        case 'accounts':
          ...
          break;
        case 'declarations':
          endpoint = `/export/declarations`;
          filename = '申报汇总.xlsx';
          break;
        case 'awardAllocation':
          endpoint = `/export/award-allocation`;
          filename = '院奖分配.xlsx';
          break;
        case 'honorDeclarations':
          endpoint = `/export/honor-declarations`;
          filename = '荣誉称号申报表.xlsx';
          break;
        case 'declarationAttachment2':
          endpoint = `/export/declaration-attachment2`;
          filename = '附件2.2024-2025学年院级奖学金、优秀学生干部、优秀学生汇总表【含填报说明】.xlsx';
          break;
        case 'mailLogs':
          endpoint = `/export/mail-logs`;
