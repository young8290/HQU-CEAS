...
export default function AwardsPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      api.get(`/awards/candidates/${classId}`),
      api.get(`/awards/allocation/${classId}`),
      api.get(`/award-declarations/class/${classId}`),
    ])
  ...
  async function submitDeclaration() {
    ...
    try {
      await api.post('/award-declarations', {
        classId,
      ...
      setMessage('奖学金申报已提交');
      setDeclaration(await api.get(`/award-declarations/class/${classId}`, { forceRefresh: true }));
    } catch (error: any) {
