    ...
    try {
      ...
      switch (importType) {
        ...
        case 'declaration_supplement':
          endpoint = user?.role === 'monitor' && user.classId ? `/declaration-supplements/import/${user.classId}` : '/declaration-supplements/import';
          break;
        case 'monitor_email':
          ...
          <div className="flex flex-wrap gap-2">
            {visibleTemplates.map((template) => (
              <button
                ...
                type="button"
                onClick={async () => {
                  await api.download(`/templates/${template.type}/download`, `${template.name}.xlsx`);
