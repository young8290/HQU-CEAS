...
export default function ScoreReviewMemberPage() {
  ...
  async function updateCheck(studentId: number, status: CheckStatus) {
    ...
    try {
      const result = await reviewApi.put(`/score-review-invites/checks/${studentId}`, { status, remark });
      setChecks((prev) => ({
        ...prev,
        [studentId]: {
          ...(prev[studentId] || {}),
  ...
  async function saveSignature(imageData: string, method: 'draw' | 'upload') {
    ...
    try {
      const signature = await reviewApi.post('/signatures', {
        signerName: session.member.name,
        method,
        purpose: 'score_review_confirmation',
        imageData,
      });
      const record = await reviewApi.post('/score-review-invites/signature', {
  ...
  return (
    <main className="min-h-screen bg-[#f7f3eb] p-4 text-neutral-900 dark:bg-neutral-950 dark:text-white md:p-6">
      <div className="mx-auto max-w-[1680px] space-y-5">
        ...
        {activeDetail && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4" onClick={() => setActiveDetail(null)}>
            <div className="w-[640px] max-w-full rounded-lg border border-[#ded6c8] bg-[#fffaf2] p-5 shadow-sm dark:border-neutral-800 dark:bg-neutral-900" onClick={(event) => event.stopPropagation()}>
